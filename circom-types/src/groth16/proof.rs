//! This module defines the [`Proof`] struct that implements de/serialization using [`serde`].
...
impl<P: Pairing + CircomArkworksPairingBridge> From<Proof<P>> for ark_groth16::Proof<P> {
...
impl<P: Pairing + CircomArkworksPairingBridge> From<ark_groth16::Proof<P>> for Proof<P> {
...
mod bls12_381_tests {
    ...
    use super::*;
    use crate::{test_utils, tests::groth16_bls12_381_kats};
    ...
    pub fn deserialize_bls12_381_proof() {
        ...
        let proof = serde_json::from_str::<Proof<Bls12_381>>(&proof_string).unwrap();
        ...
        let der_proof = serde_json::from_str::<Proof<Bls12_381>>(&ser_proof).unwrap();
...
mod bn254_tests {
    ...
    fn deserialize_bn254_proof() {
        ...
        let proof = serde_json::from_str::<Proof<Bn254>>(&proof_string).unwrap();
        ...
        //serialize and deserialize and check for equality
        let ser_proof = serde_json::to_string(&proof).unwrap();
        let der_proof = serde_json::from_str::<Proof<Bn254>>(&ser_proof).unwrap();
        assert_eq!(der_proof, proof);
    }
}
